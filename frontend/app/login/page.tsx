...
function GoogleLoginButton({ callbackUrl }: { callbackUrl: string }) {
  return (
    <button
      ...
      className="flex w-full items-center justify-center gap-3 rounded-md bg-white px-4 py-3 text-gray-900 border border-gray-300 hover:bg-gray-50 transition-colors"
    >
...
function LoginContent() {
  ...
  const [authMode, setAuthMode] = useState<'loading' | 'google' | 'oidc' | 'dev'>('loading');
  ...
  return (
    <>
      ...
      <div className="space-y-4">
        {authMode === 'google' && <GoogleLoginButton callbackUrl={callbackUrl} />}
